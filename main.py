...
def Stock_SMA(stock, country):
    ...
    try:
        ...
        df = yf.download(stock, start='2020-01-01', end=date.today(), progress=False)
    # Count SMA30 / SMA90
    SMA30 = pd.DataFrame()
    ...
    plt.xlabel('01/01/2020 - ' + current_date)
                        ...
                        def Last_Month(stock, country):
                            ...
                            try:
                                df = investpy.get_stock_historical_data(stock=stock, country=country,
                                                                        from_date='01/01/2019', to_date=current_date)
                            except:
                                df = yf.download(stock, start='2019-01-01', end=date.today(), progress=False)
                            plt.figure(figsize=(12.6, 4.6))
                            ...
                            plt.legend(loc='upper left')
                            plt.show()
                            print('Prices Last Five days of ' + stock + ' =', np.array(df['Close'][-5:][0]), ';',
